...
std::shared_ptr<const dynamic_terrain::GeographicTransform>
makeSphericalCoordinates(double headingDeg = 37.0)
{
    return std::make_shared<TestGeographicTransform>(
        52.2297, 21.0122, 123.4, headingDeg);
}
                      ...
                      std::uint64_t expectedGeneration)
{
    ...
    for (const auto &page : snapshot.pages)
    {
        ...
        auto surface = page.mesh;
        CHECK(surface);
        CHECK(surface->submeshName == page.submeshName);
        ...
        CHECK(surface->positions.size() == expectedVertices);
        CHECK(surface->normals.size() == expectedVertices);
        CHECK(surface->texCoords.size() == expectedVertices);
        CHECK(surface->indices.size() ==
        ...
        {
            CHECK(surface->indices[i] >= 0);
            CHECK(static_cast<unsigned int>(surface->indices[i]) <
                  surface->positions.size());
        }
        for (const auto &normal : surface->normals)
        {
            CHECK_NEAR(normal.Length(), 1.0, 1e-12);
            CHECK(normal.Z() > 0.0);
        }
        for (std::size_t triangle = 0; triangle < surface->indices.size(); triangle += 3u)
        {
            const auto a = surface->indices[triangle];
            const auto b = surface->indices[triangle + 1u];
            const auto c = surface->indices[triangle + 2u];
            const auto geometricNormal = (surface->positions[b] - surface->positions[a])
                .Cross(surface->positions[c] - surface->positions[a]);
            CHECK(geometricNormal.Dot(surface->normals[a]) > 0.0);
        }
...
void testPagedMeshesAndDirectUvs()
{
    ...
    for (const auto &[key, firstPage] : firstPages)
    {
        ...
        CHECK(firstPage->texture->rgb == secondPage->texture->rgb);

        auto firstSurface = firstPage->mesh;
        auto secondSurface = secondPage->mesh;
        CHECK(firstSurface);
        CHECK(secondSurface);
        CHECK(firstSurface->positions.size() == secondSurface->positions.size());
        for (unsigned int i = 0; i < firstSurface->positions.size(); ++i)
            CHECK(firstSurface->positions[i].Distance(secondSurface->positions[i]) <
                  1e-7);
    }
    CHECK(overlap == 6u);
                          ...
                          std::size_t expectedCapacity)
{
    ...
    std::vector<std::weak_ptr<const dynamic_terrain::ImageData>> images;
    ...
    for (int step = 0; step < 40; ++step)
    {
        ...
        std::weak_ptr<const dynamic_terrain::ImageData> replaced = update->pages[0].texture;
        update.reset();
...
void testRefinedPixelsSurviveRecenter()
{
    ...
    CHECK(carried->texture->rgb == page->texture->rgb);
}

void testLodBandsAndProgressiveOrder()
{
    TemporaryDirectory temp;
    auto config = makeConfig(temp.Path());
    config.visualPageTextureMaxSize = 4096;
    config.visualImageryLodTable = {{200.0, 18}, {6000.0, 16}};
    config.visualRecenterDistanceM = 0.0;
    config.visualTextureGuardM = 0.0;
    auto transform = makeSphericalCoordinates();
    auto store = std::make_shared<dynamic_terrain::TileStore>(config, transform);
    dynamic_terrain::PersistentTerrainBuilder builder(store);
    const auto center = builder.SnappedCenter(store->OriginLatDeg(), store->OriginLonDeg());
    dynamic_terrain::TerrainSnapshot snapshot;
    snapshot.geometryRect.zoom = center.z;
    snapshot.patchCenterLocal = transform->LocalFromGeodetic(
        dynamic_terrain::tileYToLat(center.y + 0.5, center.z),
        dynamic_terrain::tileXToLon(center.x + 0.5, center.z),
        transform->ElevationReference());
    for (int offset : {20, 3, 0, -3})
    {
        dynamic_terrain::TerrainPage page;
        page.key = {center.x + offset, center.y, center.z};
        snapshot.pages.push_back(std::move(page));
    }
    CHECK(builder.TargetZoomForPage(snapshot, 0) == 14);
    CHECK(builder.TargetZoomForPage(snapshot, 1) == 16);
    CHECK(builder.TargetZoomForPage(snapshot, 2) == 18);
    CHECK(builder.TargetZoomForPage(snapshot, 3) == 16);
    CHECK(builder.TargetZoomForPage(snapshot, 99) == 14);
    const auto order = builder.ProgressivePageOrder(snapshot);
    CHECK(order == std::vector<std::size_t>({2u, 1u, 3u, 0u}));

    config.visualPageTextureMaxSize = 512;
    auto limitedStore = std::make_shared<dynamic_terrain::TileStore>(config, transform);
    dynamic_terrain::PersistentTerrainBuilder limited(limitedStore);
    CHECK(limited.TargetZoomForPage(snapshot, 2) == 15);

    config.visualDetailMode = "bottom_camera_only";
    config.visualPageTextureMaxSize = 4096;
    config.visualDetailRadiusM = 200;
    config.visualDetailZoom = 17;
    auto cameraStore = std::make_shared<dynamic_terrain::TileStore>(config, transform);
    dynamic_terrain::PersistentTerrainBuilder camera(cameraStore);
    CHECK(camera.TargetZoomForPage(snapshot, 2) == 17);
    CHECK(camera.TargetZoomForPage(snapshot, 1) == 14);
}
...
void testCollisionPatchFollowsLocal2Heading()
{
         ...
         std::array<double, 4>{{0.0, 37.0, 90.0, -73.0}})
    {
        ...
        const dynamic_terrain::Vec3 localX{
            std::cos(patch->yaw), std::sin(patch->yaw), 0.0};
        const dynamic_terrain::Vec3 localY{
...
int main()
{
    try
    {
        ...
        testLodBandsAndProgressiveOrder();
