...
int main(int argc, char **argv)
{
    try
    {
        ...
        CHECK(main);
        auto *engine = gz::rendering::engine(TERRAIN_RENDER_ENGINE_LIBRARY);
        CHECK(engine);
        ...
        texture->width = 64; texture->height = 64; texture->rgb = pixels;
        page.texture = texture;
        auto mesh = std::make_shared<MeshData>();
        mesh->name = "gui_plugin_mesh"; mesh->submeshName = page.submeshName;
        for (int i = 0; i < 3; ++i)
        {
            mesh->positions.emplace_back(i == 1 ? 1 : 0, i == 2 ? 1 : 0, 0);
            mesh->normals.emplace_back(0, 0, 1); mesh->texCoords.emplace_back(0, 0); mesh->indices.push_back(i);
        }
        page.mesh = mesh;
